-module(db_gen_server_trans).

%% Karl-Johan Drougge. 8.2
...
handle_call({delete, Key}, _, Db) ->
	{reply, ok, dbBinary:delete(Key, Db)}.
...
lock() -> queue_handler:lock(). 
unlock() -> queue_handler:unlock(). 
